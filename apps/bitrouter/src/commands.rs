...
/// `bro models [--provider <id>]` — list routable models, optionally
...
pub async fn list_models(
    ...
    // this turns `bro models` from "empty" into a useful catalog
    ...
    let entry: bitrouter_providers::ProviderEntry = match builtin::find(provider_id) {
        ...
        None => {
            ...
            let provider = data
                ...
                .ok_or_else(|| {
                    anyhow::anyhow!(
                        "unknown provider '{provider_id}' — try `{} providers list` \
                         for the catalog",
                        invocation::name()
                    )
                })?;
...
/// `bro skills list` — installed skills under the chosen root.
