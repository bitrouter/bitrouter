//! Explicit local Laya extension composition; stock `bro` stays generic.
...
#[derive(Parser)]
#[command(name = "bro-laya")]
struct Args {
    /// BitRouter YAML config with an explicit loopback Laya provider and
    /// LAYA_LOCAL_TOKEN; no public registry route is auto-activated.
    #[arg(short, long)]
    config: PathBuf,
...
async fn main() -> Result<()> {
    ...
    bitrouter_laya_extension::register(&mut extensions)
        .context("register local Laya Typed Decisions format")?;
    ...
    served.context("serve local Laya evaluation host")?;
    Ok(())
}
