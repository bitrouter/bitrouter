...
> [`OSS_MCP_BOUNDARY_SPEC.md`](OSS_MCP_BOUNDARY_SPEC.md) removed the OSS
> origin-server portions of this design, including `bitrouter-mcp`, `bro mcp
> serve`, and `/mcp-control`. The retained upstream-client and gateway protocol
> behavior, including its version and lifecycle notes, remains current.

This historical migration record covers BitRouter's move from `rmcp` 2.2.0 to
the 3.x line for its former **origin server** (`crates/bitrouter-mcp`) and its
retained **gateway client** (`crates/bitrouter-sdk/src/mcp`) under MCP protocol
version `2026-07-28`.

**Branch:** `claude/bitrouter-mcp-rust-sdk-36c540`.
**Merge gate:** originally met on stable `rmcp` 3.1.0; the dependency floor is
now 3.3.0 (see [Merge gate](#merge-gate)).
...
draft). **2026-09-14 update:** the follow-up Skills conformance work raised the
SDK floor to rmcp 3.3.0 and changed `mcp.upstream_protocol` to default to
`auto` (modern discovery with classified legacy fallback). Explicit `latest`
still forces the historical `2025-11-25` initialized lifecycle. See
...
`auto` (the default) and `"2026-07-28"` use
`ClientLifecycleMode::Auto`: they open with `server/discover`, send
self-contained metadata on later requests, and use rmcp's classified legacy
fallback. `latest` explicitly keeps the legacy `initialize` path at rmcp
`LATEST` (`2025-11-25`).

In rmcp 3.3 this is a classified fallback, not a `METHOD_NOT_FOUND`-only rule.
It retries legacy `initialize` after a 10-second discovery timeout or a
complete, correlated JSON-RPC error other than the modern rejection codes.
Transport failures, malformed or uncorrelated replies, version-negotiation
failure, and modern security/metadata rejections remain errors. Explicit
`latest` remains available for an upstream known to need the legacy lifecycle.
...
- **SEP-2243 custom tool-parameter headers.** The gateway resolves the
  requested tool's `inputSchema`, follows statically reachable `properties`
  annotations, and validates every `Mcp-Param-*` value against the body before
  dispatch. Missing, mismatched, spurious, unreachable, invalid, or duplicate
  annotations return HTTP 400 / JSON-RPC `-32020`.
...
`server/discover` with rmcp 3.3's classified fallback. A discovery timeout or
a complete, correlated non-modern JSON-RPC error retries `initialize`;
transport failures, malformed or uncorrelated responses, and modern rejection
codes do not silently downgrade. On the `2026-07-28` path there is no correct
modern alternative, while explicit `latest` remains the forced legacy path.
...
BitRouter — long-running *upstream* tool calls and the fleet spawn surface both
want it — but it introduces a polling lifecycle, TTL handling, and cancellation
...
**Superseded 2026-09-14: default to `auto`.** rmcp 3.3's discovery and fallback
behavior is now covered by real-transport tests, and native Skills discovery
requires the modern path. Stock config therefore prefers `2026-07-28`; users
can force legacy initialization with explicit `latest`. Tests pin both
mappings so a dependency bump cannot change them silently. rmcp 3.3 `Auto`
falls back on its discovery timeout or a correlated non-modern JSON-RPC error;
it does not fall back after transport, correlation, malformed-response, or
modern rejection failures.
...
3. New coverage — 25 tests:
   ...
   - In the original 3.1 rollout, `mcp.upstream_protocol` defaulted to `latest`.
     The 2026-09-14 follow-up supersedes that assertion with `auto`, retains
     explicit `latest`/`"2026-07-28"`, and still rejects unknown values.
...
5. No change to `skills/bitrouter/` or the plugin manifests. Re-verified after
   ...
   `bro mcp serve`, unchanged.

## Merge gate

**The gate is met.** Stable `rmcp` 3.0.0 shipped on 2026-07-28, and the first
implementation was accepted against 3.1.0. The 2026-09-14 Skills follow-up
raised the workspace requirement to `rmcp = "3.3"`; the single lockfile
resolution is now rmcp/rmcp-macros 3.3.0 with `process-wrap` 10.0.0. A released
specification alone was not sufficient to close this gate: the stable SDK and
its acceptance evidence were required.

1. `Cargo.toml` uses the stable `3.3` caret requirement and `Cargo.lock`
   resolves rmcp/rmcp-macros 3.3.0.
2. The releases through 3.3.0 were reviewed, including MRTR metadata decoding,
   discovery timeout/classification, protocol validation, version negotiation,
   Streamable HTTP concurrency, and legacy fallback corrections.
3. The original stable-dependency gate passed at **2109 passed, 11 skipped**;
   the dated native-Skills conformance plan records the newer full-workspace
   acceptance evidence rather than overwriting that historical result here.

The 2026-09-14 follow-up changed `mcp.upstream_protocol` to default to `auto`
after upgrading to rmcp 3.3 and adding modern/legacy real-transport coverage.
Task-returning upstreams still become explicit errors per D1; `latest` remains
the operator escape hatch for a server that must stay on legacy initialization.
