...
- **模型（Models）** — 跨提供商、跨协议、跨账户地路由 LLM 调用。参见 [Models](/docs/concepts/models)。
...
成本优化是用仍能达成目标的最便宜的模型、工具*与*子智能体。每个循环都有一份 [策略（policy）](/docs/concepts/policy)，BitRouter 依据实时的成本与结果信号对其调优，因此运行越久就越便宜。
...
- **效率** — 价格感知路由按任务复杂度把每次调用匹配到合适的模型，让琐碎调用不再按前沿价格计费。
