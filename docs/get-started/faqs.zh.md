...
BitRouter 与下面这些网关都路由 LLM 流量。差别在于它们*路由什么*、*优化什么*——BitRouter 是唯一一个把**模型、工具与智能体视为单一可路由面**、并按成本优化整个生产**循环**的方案。
...
TensorZero（Rust）优化的是**模型本身**——提示词、权重、模型选择。BitRouter 优化的是**整个循环**，按成本，跨越模型、工具与子 Agent（MCP + ACP）。此外 TensorZero 已停止维护。
