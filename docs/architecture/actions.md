...
| `route_preview` config fallback ran the policy table while `bitrouter route` did not | Historical `apps/bitrouter/src/routing_preview.rs:131` vs [`commands.rs:215`](../../apps/bitrouter/src/commands.rs:215) |
| …and their report keys differ | `requested_model` / `provider_chain[].api_protocol` vs `model` / `chain[].protocol` |
| `route_preview` snapshots config **once, at `mcp serve` start** | The former `RoutingPreview::new` was called during `McpAction::Serve` wiring ([`main.rs:2260`](../../apps/bitrouter/src/main.rs:2260)); a `bitrouter.yaml` edit was invisible to a long-lived server that the CLI would pick up |
...
| A fourth frontmatter reader existed | The former `skill_body` was hand-rolled beside `extract_frontmatter_block` ([`format.rs:53`](../../apps/bitrouter/src/skills/format.rs:53)) |
...
| `skills/bitrouter/references/cli.md` documents `mcp search/list/add` but never `mcp serve`'s tools | `skills/bitrouter/references/cli.md:298`–`324` |
...
The TUI is a third consumer of (c) only. Its charter (retired ACP TUI design §8.3:
...
- Fix [`README.md:95`](../../crates/bitrouter-mcp/README.md:95): point at
  `skills/bitrouter/references/cli.md`'s new origin-server section, not a file that never existed.
- Add a `bitrouter mcp serve` section to `skills/bitrouter/references/cli.md`: transports, backends,
...
- **~~Regression: `status` loses the cost footer.~~ Resolved by the spend
  ...
  `CallToolResult` is needed. `skills/bitrouter/references/cli.md`, the skill, and the code comment all
  say this now.
...
- The TUI's data model. Its session-only charter (retired ACP TUI design §8.3) is
