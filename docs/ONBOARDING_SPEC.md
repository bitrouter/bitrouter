# Spec: `bro` onboarding — a guided CLI wizard over existing verbs
...
1. One guided entry: bare `bro` when unconfigured, and `bro init`
...
```
bro                      # no subcommand: run wizard IFF unconfigured; else status + hint
...
- clap change: `Cli.command` becomes `Option<Command>` (`main.rs:58-59`). `None`
  ...
  (configured). Bare `bro` never re-onboards a configured user and never
...
- [ ] `skills/bitrouter/SKILL.md` — revise the "no interactive setup wizard —
      onboarding is two commands" line; document bare `bro` + `bitrouter
...
- [ ] `.claude-plugin/`, `.codex-plugin/`, `.agents/plugins/marketplace.json` —
      ...
      unaffected; confirm no manifest invokes bare `bro` or `init`).
...
- **Integration** — bare `bro` unconfigured → wizard entry; configured →
...
- **E2E** (bitrouter-e2e-test skill) — fresh `HOME`: `bro init --yes
  ...
  present-or-reported, JSON envelope emitted; then bare `bro` → status,
  not the wizard.

## 11. Acceptance criteria (v1 done)

- [ ] Bare `bro` launches the wizard when and only when the probe reports
...
5. **Entry: bare `bro` (unconfigured) + `bro init` (re-run)**;
...
1. Bare-configured `bro` output — a compact `status` view, or clap's help?
