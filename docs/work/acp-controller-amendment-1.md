...
[`ACP controller architecture`](../architecture/acp-controller.md)
...
Gated by the [`ACP safety invariants`](../invariants/acp.md), which
...
The retired ACP TUI design reached these cases through `Scope::Wider`. Under this
...
9. ~~The [chat state machine](../architecture/chat-machine.md),
...
| `cli.md:198`, `skills/bitrouter/references/cli.md:344` | the `all callers` cost line — already false for `chat` |
| `cli.md:200`, `skills/bitrouter/references/cli.md:346` | `/route` gated on an attributable launch id; re-reads `providers/list` |
...
| `skills/bitrouter/references/cli.md:304` | `.bitrouter/sessions/` — **already stale**, no such path exists |
| `docs/architecture/overview.md:29-30`, `:46-63` | ACP as "down / engine / up"; the `costScope` key |
| Retired ACP TUI design, Decision 4 | The wider-scope requirement — superseded by C2/C5 |
| `docs/architecture/chat-machine.md` §2.3, §4.2 | Written against `PendingPermission` and `Session::prompt` — **rewritten**, both conclusions intact |
