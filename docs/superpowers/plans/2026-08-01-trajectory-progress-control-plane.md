...
- **Fifteenth independent specification verdict:** PASS, 0 Critical / 0 Important / 1 Minor. Independent call-site and malicious-extension probes confirm all five opaque auth-extension failure exits discard original Display, Debug, and source-chain text at the owning boundary, successful authority/refresh/dispatch behavior remains intact, and the previously closed continuation privacy/finalizer/registry/restart/shutdown/no-panic contracts remain GREEN. The sole Minor is the already disclosed required `source_protocol` public/serde migration.
- **Fifteenth independent quality verdict:** FAIL, 0 Critical / 1 Important / 3 Minor. The privacy finding is closed, but rebuilding every dynamic-auth error as `Internal` destroys safe routing semantics. An independent two-target Pipeline probe returned `Upstream { status: 503, ... }` from the first target's applier; the candidate surfaced `Internal`, `DefaultFallbackPolicy` did not advance, and the healthy second target was called zero times, while the same target succeeded alone. Minors are the disclosed `source_protocol` migration, the newly disclosed required public `Config.continuation` source migration, and secret-bearing derived Debug output from private resolved continuation state and public applied-auth requests. Every push remains blocked.
- **Sixteenth-round boundary:** Preserve the semantic class needed by routing and public status while discarding every opaque extension-controlled diagnostic. Rebuild each `BitrouterError` variant with a fixed operation-specific message or text-free safe form; preserve only bounded scalar control data such as upstream status, retry-after, and the payment/rate/timeout/unavailable category. Discard JSON payloads, detail strings, challenges, scopes, URLs, and source chains. Add a real two-target fallback matrix for streaming and non-streaming auth application/refresh failures, including availability and payment exhaustion, plus fail-fast controls. Replace secret-bearing Debug derivations with explicit redacted implementations. Do not add provider/task/benchmark-specific branches or change successful authority behavior.
- [x] **Step 73: RED-test semantic fallback preservation and Debug redaction.** The real all-feature SDK two-target Pipeline matrix failed on exact `a7e0ae67` at the first non-streaming 503 apply case: the expected healthy fallback returned `Internal("upstream authentication failed")` instead, matching the independent quality probe. Separate variant and fail-fast controls failed because a 503 became `Internal` and an `Unauthorized` 401 became status 500. `AppliedAuth` Debug failed by exposing the authenticated URL path and `api_key` query, while `ResolvedContinuation` Debug failed by exposing both the native provider response id and target fingerprint. All failures are assertion-level behavioral REDs; the fallback test used the authorized localhost boundary and the other three required no network.
- [x] **Step 74: Normalize opaque errors without erasing safe control semantics.** One exhaustive host-owned normalizer now covers every `BitrouterError` variant and all five dynamic-auth extension exits. It preserves the variant, status, retry-after, and text-free payment/rate/timeout/unavailable categories used by routing while replacing text/JSON-bearing fields with the fixed operation diagnostic and clearing provider detail, challenge, and scope fields; no source error is attached. Static transport and successful typed authority are unchanged. `AppliedAuth` Debug omits the entire authenticated URL, and `ResolvedContinuation` Debug redacts both native id and target fingerprint. The real non-stream/stream two-target prepare/apply/payment/post-401-refresh matrix passes 1/1; exhaustive kind/status/privacy plus fail-fast and applied-auth Debug controls pass 4/4; prior SDK opaque privacy/settlement and positive 401 refresh controls pass 4/4; App Debug/mapped-authority/public-runtime/restart-authority controls pass 4/4. Full exact-tree gates remain pending.
- [x] **Step 75: Rerun the new fallback/privacy/Debug matrix, complete continuation/App/SDK/workspace gates, Clippy, formatting, dist-helper, and every static audit.** Focused sixteenth-round fallback/classification/privacy/Debug passes 5/5; prior SDK opaque privacy/settlement and positive 401 refresh passes 4/4; App Debug/mapped-authority/public-runtime/restart-authority passes 4/4. Complete continuation passes 92/92; all-feature SDK passes 791/791 with 2 skipped; all-feature App passes 1,201/1,201 with 9 skipped; the final serial all-feature workspace run passes 2,513/2,513 with 11 skipped. Its first compile attempt never entered tests because the current worktree had only 117MiB free and rustc reported `No space left on device`; deleting only that worktree's 21GB regenerable Cargo incremental cache restored 15GiB, and the exact command then passed. Formatting, diff check, strict workspace all-feature/all-target Clippy, and dist-helper pass; schema and registry remain current at 50 providers / 49 canonical models. Static audits find exactly five intended tracked files, no untracked file, migration, runtime artifact, production sentinel, task/benchmark/provider-specific runtime branch, global secret registry, added production panic, or automatic signal path. The tree is intentionally dirty only until the signed Step 76 commit.
- [ ] **Step 76: Create one signed sixteenth-round child without rewriting history, build a fresh sixteen-commit package, and require fresh specification PASS followed by separate quality PASS before any push.**
