...
The `otel` feature is the case in point, and it is worth stating precisely, because the loose version of this sentence is an argument for moving almost anything in. The contract is the **span schema** — the span names (`chat`, `route`, `settle`, the per-hop `chat`), the `bitrouter.*` attribute vocabulary, and the invariants that fail silently when a deployment re-derives them wrong (a hop is not a `gen_ai` generation; stamping it as one makes gen_ai-aware backends double-count cost). *That* is BitRouter semantics. The OTLP exporter ships alongside it because there is exactly one renderer of that schema and it is default-off — not because OTLP transport, bearer refresh, batch processing or endpoint configuration are SDK concerns in their own right. By volume they are most of `crates/bitrouter-sdk/src/otel/`: roughly 40% of its production lines are transport and vendor glue and another 17% is deployment configuration, against ~43% actual span semantics. That ratio is the price of keeping the one renderer next to the schema it renders. It is not a precedent.

Because the schema is the contract, it is written down rather than inferred from the exporter's call sites. `crates/bitrouter-sdk/src/otel/schema.rs` declares every span, attribute, event, metric and silent-failure invariant, names no `opentelemetry` type, and renders to the committed artifact `crates/bitrouter-sdk/span-schema.json` — regenerate with `UPDATE_SPAN_SCHEMA=1 cargo test -p bitrouter-sdk --all-features committed_artifact`, and the ordinary test run fails when it is stale. (It is not a `dist-helper` artifact under `dist/`: rendering it there would put the OpenTelemetry stack into that helper's own dependency tree, which the `feature-isolation` job forbids by name. It sits beside `public-api-deps.txt` instead, the crate's other generated manifest — but unlike that one it *ships* with the crate, because it is the interop surface.) Conformance tests in `otel/exporter.rs` and `otel/acp.rs` drive real lifecycles and assert that nothing reaches the wire the declaration does not describe, so the artifact is checked rather than aspirational. See [`OTEL_TIERING_SPEC.md`](OTEL_TIERING_SPEC.md).
