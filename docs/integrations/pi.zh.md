...
每个 `models[].id` 都是 `provider/model` 形式的注册表 id。在 BitRouter Cloud 上，可附带 `:cost` / `:latency` 这类托管路由 profile；在本地 OSS 守护进程上，请先在 `bitrouter.yaml` 中定义对应的 `variants`。详见[模型](/docs/concepts/models)。
