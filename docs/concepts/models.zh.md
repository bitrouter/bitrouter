---
title: Models
description: 在 BitRouter 上，模型是由众多供应商共同提供的聚合体——通过四种协议触达、逐请求排序，并支持可选的托管 profile。
...
正因为模型是聚合体，请求它会触发一次**供应商选择**步骤。在 BitRouter Cloud 与兼容的托管路由部署中，默认会按一个均衡评分对符合条件的供应商排序——综合成本、延迟、吞吐与可用性——并把请求发往最优者。本地 OSS 守护进程使用路由配置中的供应商顺序；除非你配置 virtual-model strategies、variants 或 presets，否则默认是确定性的供应商名称顺序。当所选供应商出现瞬时故障时，BitRouter 可以回退到下一个排名的供应商，或回退到你列出的下一个模型。

## 变体为单次请求重新排序

当一个模型有多个供应商时，你有时希望为某一次调用调整这个排序。在 BitRouter Cloud 与兼容的托管路由部署中，内置的**模型变体**后缀——`:cost`、`:latency`、`:throughput`——会仅针对该请求、沿你指定的轴对*符合条件*的供应商重新排序。它们从不改变哪些供应商符合条件，也从不改变授权；不带后缀的裸 id 就是默认的均衡策略。

对于本地 OSS 守护进程，后缀由配置定义：请先在 `bitrouter.yaml` 的 `variants` 下添加条目，再使用 `:<variant>` 选择器。未知后缀会保留为字面模型 ID 的一部分，通常会得到 404。

## 开源模型，享折扣

在 BitRouter Cloud 上，开源（非闭源）模型还具备第二个特性：BitRouter 通过自托管供应商以**默认低于官方定价 25%** 的价格提供它们，无需任何后缀或配置。`:discount` 后缀会把请求显式钉到该供给上，账号上的任何自定义折扣也在此生效。

## 了解如何使用

- [供应商选择](/docs/features/provider-selection) —— 一个模型背后的供应商如何排序。
- [模型回退](/docs/features/model-fallback) —— 传入有序列表，失败时逐一回退。
- [模型变体](/docs/features/model-variants) —— 托管 `:cost` / `:latency` / `:throughput` 后缀，以及本地配置定义的变体。
