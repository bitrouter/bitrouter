---
title: 模型变体
description: 在 BitRouter Cloud 中，在模型 ID 后追加 :cost、:latency 或 :throughput，即可内联选择供应商排序方式——逐请求、无需任何请求体字段。
sourceHash: 9704500065fd7a0150d6564893f63556411fefc50300238ffb7d68e85410b8eb
---

当一个模型由多个供应商承载时，BitRouter 必须为每个请求挑选发往哪个上游。在 BitRouter Cloud 与兼容的托管路由部署中，**模型变体**让你内联做出这个选择：在模型 ID 后追加一个 `:<profile>` 后缀，BitRouter 就会按你指定的维度对供应商排序。

后缀本身就是 `model` 字符串的一部分，因此既不需要请求体字段、也不需要 SDK——在托管路由表面上，它在 OpenAI、Anthropic、Google 三套接口上行为一致。

<Callout type="info">
对于本地 OSS 守护进程，profile 后缀不是内置能力。本地请求只有在 `bitrouter.yaml` 中定义了对应 `:<variant>` 时才会剥离该后缀；否则后缀会保留为字面模型 ID 的一部分，通常会得到 404。
</Callout>
...
除了上面的排序 profile，BitRouter Cloud 还接受一个 `:discount` 后缀，把请求路由到 BitRouter 的**自托管供应商**——开放模型在那里以低于官方的价格提供：
...
所选 profile 会记录在每次托管请求上，并在用量历史中返回。`GET /v1/namespaces/{nsid}/requests` 的每一行都带有 `routing_profile` 字段（`balanced` / `cost` / `latency` / `throughput`），便于你审计各策略各自占了多少流量。

## 与 `provider.sort` 的关系

在托管路由表面上，目前选择路由 profile 的受支持方式就是模型 ID 后缀。请求体形式的等价物 `provider.sort` 在 [供应商选择](/docs/features/provider-selection) 中有描述，但**尚未启用**；在它上线之前，请使用后缀。
