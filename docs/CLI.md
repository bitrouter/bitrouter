...
Launches the **composite multi-agent TUI**, which has two screens. The **default screen** is the focused agent at full terminal width over a one-line status bar — no sidebars and no rail, so the harness's own TUI gets every column it asks for. The **manager screen** (`Ctrl-Space m`) is a full-screen overlay listing the whole fleet in one list — orchestrator PTY sessions and ACP subagents together, sorted by who needs you — and carries the supervision verbs (`y`/`a`/`n` decide, `D`/`m`/`p`/`r` review, `c` close) against its cursor row. Because a focused PTY pane passes every key to the child, the manager is the only surface from which an agent that is not on screen can be unblocked. The viewport holds exactly one agent; there are no splits. `--agent claude|codex|…` hosts that harness's real native TUI on a PTY as the *orchestrator*; a configured `agents:` id renders that ACP agent as a **read-only `Monitor`** — **there is no input bar anywhere**: the human never types into a subagent (the orchestrator steers subagents via the injected fleet MCP tools; `bitrouter serve` should be running alongside — the bar's `serve ●/✗` dot is live).
