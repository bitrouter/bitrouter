...
Companion to the [`ACP controller architecture`](../architecture/acp-controller.md). That document
...
|---|---|---|---|
| A manager that detaches and reattaches sees still-outstanding permissions replayed | `PermissionRegistry` — **deleted** with the engine | its two tests went with it | Reattach is not in the controller model. The ACP controller architecture §9 instructs managers to assume nothing survives reconnect. Irrelevant to `chat`, which is one process and one connection. |
