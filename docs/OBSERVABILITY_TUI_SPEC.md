...
Supersedes the CLI framing of #782 (`bro top`, `bro tui` deprecation).
...
lines), `fleet_mcp.rs`, `fleet.rs`, and the `bro tui` command are gone,
...
- Non-tty stdout with `--watch` → print **one** snapshot as a table and exit,
  ...
  worse default.
- Rejected: `bro top` (accurate for v1's stream, misleading the moment it
...
`bro tui` was **already removed** by #786 — there is no deprecation
...
| 2 | **`status --watch`**, not `bro top` | `status` already exists as a script-parseable probe and stays byte-identical; `--watch` is the established idiom for "the live version of this"; `top` would be accurate for v1 and misleading once it manages. Defers the naming call to when the identity is proven. |
