...
LiteLLM 的代理需要你自己运维。BitRouter 的**托管云与本地二进制对外暴露同一个 OpenAI 兼容端点** —— 开发期可在本地启动，生产期切换到 `api.bitrouter.ai`（反之亦然），客户端代码不需要修改。CLI、向导和 Agent Skills 在两种模式下都能使用；在设置 TUI 中按一个键即可切换。两种流程详见[快速开始](/docs/get-started/quickstart)。
