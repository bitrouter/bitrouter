...
description: >
  ...
  credentials, wiring a coding agent with bro launch, routing, and spend.
...
from the `bro` CLI. No BitRouter account is required to run it — where the
...
bro status             # prints `running: no` when no daemon is reachable
...
→ `bro start`, then §5; both → §5, the harness is all that is left. Do not
...
Every installer lands the CLI as **`bro`** (deprecated `bitrouter` alias, gone in 1.0.0).
Verify with `bro --version`; on failure read `references/diagnose.md`.

### 3. Configure — drive the headless wizard

`bro init --yes` is the scriptable onboarding path: it never blocks on a
...
```bash
bro init --yes --use-detected --harness claude --after launch
```
...
explicit `bitrouter.yaml` and check it with `bro config validate -c
...
`bro launch` routes a harness's native TUI without editing its config
files — reversible, per-process, and it auto-starts the daemon if it is down.

```bash
bro launch -a claude
bro launch -a codex -- -p "summarize this repo"
```
...
`bro launch -a claude` (or restart the harness with the env override) to
route this session." One exception: the origin MCP server (`bro mcp
...
and send them to `bro mcp --help`.

For a programmatic ACP manager, use `bro spawn claude-acp --serve` or
`bro spawn codex-acp --serve`. Stable ACP v1 on exact adapter pins,
...
```bash
bro route claude-sonnet-4-6   # resolve a model through the routing table
bro models                    # everything routable
bro status --requests         # settled requests + spend, JSON (--human for a table)
```
...
| `references/sessions.md`, `references/updating.md` | ACP controller, served vs in-process (`acp serve\|prompt`, native sessions, NDJSON, `bro chat <agent>`); `bro update` and channels |
