...
| `bitrouter verify-exchange <inference-id> --model <model> --resource <path> [--response <text>]` | **Chainlink only. Requires `--features chainlink-demo` at build time.** L1.5 exchange-integrity check: re-hashes the uploaded resource file and compares it against the `digest` the Chainlink service reported. Prints `digests_consistent: true/false` and always prints `TEE-attested: ✗` with a note that the Chainlink dev-preview exposes no signed attestation quote. **`verified` is always `false`** — nothing is signed, so no cryptographic claim is made. The command never claims "TEE-attested". **Env vars:** `CHAINLINK_CONFIDENTIAL_API_KEY` (required); `CHAINLINK_BASE` (optional, defaults to `https://confidential-ai-dev-preview.cldev.cloud`). |
