...
| `bitrouter mcp search <query> [--limit N]` | Search the official MCP registry (registry.modelcontextprotocol.io) server-side. Rows: name / version / install support (`remote` zero-install, `npx`/`uvx` stub-able, `manual` oci/mcpb-only) / description. Only `active` + latest-version entries. |
