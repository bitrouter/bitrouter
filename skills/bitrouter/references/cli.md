...
Manage targets with `bro context add NAME --endpoint URL --token-env
...
| `bro mcp serve` | Run BitRouter's local origin MCP server over protocol-pure stdio for a host or plugin. Network-capable hosts connect directly to the daemon's authenticated `/mcp-control` Streamable HTTP endpoint. The old standalone HTTP/cloud flags are hidden compatibility inputs and no longer start a second listener. |
...
| `bro agents inspect <agent> [--config PATH]` | Open a fresh ACP session, wait briefly for advertised slash commands, and report which source answers each command. |
