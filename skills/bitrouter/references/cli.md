...
| Command | Effect |
|---|---|
| `bitrouter serve [--config PATH]` | Run the inference HTTP server + local control socket **in the foreground**. Ctrl-C to stop. Optional `control.enabled: true` also starts the read-only remote-control API at `control.listen` (default `127.0.0.1:4358`); it requires a dedicated `BITROUTER_CONTROL_TOKEN` of at least 32 bytes, stays loopback-only for a private tunnel/TLS reverse proxy, and ignores inference `server.skip_auth`. This HTTP-only MVP exposes capabilities/status/models/route preview, not ACP sessions. |
...
Two ACP execution modes share one controller and differ only in who drives it. `acp serve` exposes the connection-level controller over **stdio** for an external manager. `run` drives one prompt **in-process** and presents it as NDJSON/text/quiet output. Session ownership is harness-native in both. `bitrouter spawn <agent> --serve|-p` and `acp serve|prompt` remain compatibility/integration forms. Both **attempt to route the harness's model traffic through the daemon when the headless adapter supports redirection** — add `--direct` / `--base-url` / `--model` / `--no-start` (see "Harness launch & spawn").

| Command | Effect |
|---|---|
| `bitrouter run <agent> <prompt> [--turn-timeout SECS] [--no-wait] [--approve-all\|--approve-reads\|--deny-all] [--permission-policy JSON\|@PATH] [--format json\|text\|quiet] [--direct] [--base-url URL] [--model ID] [--no-start] [--config PATH]` | Canonical always-headless agent entry. Launch one session, send one prompt, and stream NDJSON by default. The options and output contract are the same as `acp prompt`; `spawn <agent> -p` is the compatibility spelling. |
...
| `bitrouter acp prompt --agent <id> [--turn-timeout SECS] [--no-wait] [--approve-all\|--approve-reads\|--deny-all] [--permission-policy JSON\|@PATH] [--format json\|text\|quiet] [--direct] [--base-url URL] [--model ID] [--no-start] [--config PATH] <text>` | Compatibility form of `run`: launch a session, send one prompt, and stream session updates to **stdout as NDJSON** (one JSON object per line), then exit. First line is the `session` correlation line (see below). Logs go to stderr. `--no-wait` submits and returns `{"type":"submitted"}` without streaming. `--turn-timeout` is enforced here by the client: on elapse the harness gets `session/cancel` and three seconds to comply, then the turn fails. Attempts to route through the daemon when the headless adapter supports redirection (`--direct` opts out). Permission requests are answered by the flagged policy (see **Headless permissions** below); `--format text` prints the transcript as `tui <agent>` does to a pipe, `--format quiet` only the assistant text. |
...
## Interactive TUI (`bitrouter tui`)

Bare `bitrouter tui` is the operations dashboard: Overview, Models, Requests,
and Route pages over the same local or `--context`-selected actions as the CLI.
It refreshes read views every two seconds and performs no mutation. `bitrouter
tui <agent>` is the interactive counterpart to `acp serve`: same launch, same
routing flags, same per-session log, but rendered for a person instead of
exposed to a manager over stdio.

| Command | What it does |
|---|---|
| `bitrouter tui [<agent>] [--turn-timeout SECS] [--direct] [--base-url URL] [--model ID] [--no-start] [--config PATH]` | With no agent, open the local/remote operations dashboard. With an agent, render one ACP session in the terminal: streamed messages, reasoning, tool calls with diffs, permission prompts, context usage, and attributed cost. The session runs the same in-process controller `run` does and draws inline into real scrollback. `bitrouter chat <agent>` is a hidden compatibility alias. |
...
tui <agent>` and `bitrouter run`. There is deliberately no key that runs
...
**On failure**: a failed turn prints the tail of `~/.bitrouter/logs/session-<stamp>-<pid>.log` after the session ends and names the path. There is no permanent log pane. `tui <agent>` logs **only** to that file — it owns the terminal, so a stray log line would scroll the screen out from under the renderer.
