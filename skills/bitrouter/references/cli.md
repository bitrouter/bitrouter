...
| `bitrouter mcp serve [--transport stdio\|http] [--backend local\|cloud\|skills] [--local-url URL] [--cloud-url URL] [--token TOKEN] [--bind ADDR]` | Run BitRouter as an **origin** MCP server (the inverse of `bitrouter tools`). Long-running: stdout is the JSON-RPC wire, not a result envelope. Transports: `stdio` (default; what a client launches as a subprocess) and `http` (streamable HTTP at `/mcp-control`, default bind `127.0.0.1:4357`). Backends: `local` (stdio default — the BYOK daemon at `--local-url`, default `http://127.0.0.1:4356`; unauthenticated, so `http` on it refuses a non-loopback `--bind`), `cloud` (http default — `--cloud-url`, default `https://api.bitrouter.ai`; stdio uses `--token`/`BITROUTER_TOKEN`, http is multi-tenant and forwards each client's own bearer so `--token` is ignored and a missing bearer is `401`), `skills` (stdio only — the installed-skills tree under the current directory). **Control and introspection only — there is no inference tool.** To run a completion, call the daemon's HTTP API (`/v1/chat/completions`, `/v1/messages`); this server tells you what to send there. Tools: `list_models` everywhere; `status` on stdio+local and any cloud profile (not on http+local — only a process on the daemon's own machine can read its control socket); `route_preview` on stdio+local; `skills_search` / `skills_get` on **every stdio profile** (which also serve SEP-2640 `skills/list`/`skills/get` plus `resources/*`) — so a `bitrouter mcp install`-ed client, which launches plain `mcp serve`, now sees skills; `--backend skills` is the narrow gateway-subprocess profile that carries nothing else. Only wired capabilities register tools, so an HTTP client never sees `route_preview` or the skills tools. `status` returns the same report type as `bitrouter status`, advertised as the tool's `output_schema`; a stopped daemon is `running: false`, not a tool error, and it reports the same `spend` block the CLI does. `list_models` likewise returns the same report as `bitrouter models`: every model with **all** its providers (the fallback chain — it used to keep only the first), an optional `provider` filter matching `--provider`, and a `resolved_via` of `live` or `config`. On stdio+local it reads the daemon's live routing table over the control socket and falls back to a config parse, so it answers **with no daemon running**; other profiles use the backend's `GET /v1/models`, which does need one. `route_preview` returns the same report type as `bitrouter route` — read `effective_model`, not `requested_model` — and resolves config **per call**, so an edited `bitrouter.yaml` is visible without restarting the server. Spend reaches a client as typed structured content under `status`'s `spend` — the same metering database `bitrouter status` and `bitrouter status --requests` read. |
| `bitrouter mcp install --client claude\|cursor [--config PATH]` | Render the MCP client config block that launches `bitrouter mcp serve` over stdio. With `--config`, merge it into that file; without, print it. |
...
Non-TTY JSON, binary bodies, and SSE stream byte-for-byte to stdout. HTTP 4xx/5xx preserves the body on stdout, writes the error to stderr, and exits non-zero. The endpoint must be relative to the signed-in Cloud origin; available paths are governed by the Cloud API contract and the credential's scopes.
...
*cannot* reach (`opencode`, `pi`, `hermes`, `openclaw`) by synthesizing a
throwaway config file — headless `spawn` still runs those direct with a note.

| Command | Effect |
|---|---|
| `bitrouter launch --agent <id> [--model ID] [--config PATH] [--base-url URL] [--no-install] [--no-start] [--check] -- <agent args...>` | Launch a coding-agent CLI's native TUI through BitRouter without editing agent config files. `--agent` accepts any catalog harness with an interactive binary: `claude`, `codex`, `opencode`, `pi`, `hermes`, `openclaw`, `grok`, and `agy` (catalog ids `claude-acp`, `codex-acp`, `pi-acp`, `hermes-acp`, and `antigravity` also resolve); an unknown id fails up front with the available list. Claude uses child env overrides (`ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`); Codex uses one-shot `-c` provider overrides with `wire_api="responses"`; **opencode, pi, hermes, and openclaw route via synthesized config** under the working tree's self-ignoring `.bitrouter/launch/`, with model lists filled best-effort from the daemon's `/v1/models`. `grok` and `agy` are own-auth launches, so they are not redirected or metered. `--model ID` pins the model through whatever mechanism the harness has (model env var / `-c model=` / the synthesized config's default). **Gateway MCP injection**: the `bitrouter_tools` server (the daemon's aggregate `/mcp` route, omitted when `mcp.aggregate.enabled: false`) and the `bitrouter_skills` server (`mcp serve --backend skills`) are injected into harnesses that have a mechanism for it — claude (`--mcp-config`), codex (`-c mcp_servers…`), opencode and hermes (synthesized config); pi, openclaw, grok, and agy have no injectable MCP surface and launch without them. Only `claude` and `codex` have a bundled native installer; the others error with a pointer upstream. Prints a one-line session spend summary to stderr on exit. |
| `bitrouter spawn <agent> -p "<text>" [--no-wait] [--result-schema JSON\|@PATH] [--approve-all\|--approve-reads\|--deny-all] [--permission-policy JSON\|@PATH] [--format json\|text\|quiet] [session/routing flags]` | Spawn an ACP sub-agent, send one prompt, stream **NDJSON** to stdout, exit. `<agent>` is a catalog id (`claude-acp`, `codex-acp`, `gemini-cli`, `opencode`, `pi-acp`, `hermes-acp`, `openclaw`) or a configured `agents:` entry; a catalog id needs no config entry. `--result-schema` adds the machine-consumable result contract (see **Result contract** above); the permission and format flags are **Headless permissions** above and apply to `-p` only. |
...
- Fail-fast: if the daemon is unreachable (after auto-start) or auth is required and absent, `spawn` emits a single structured error **before** any session side effect — NDJSON `{"type":"error","code":"daemon_unreachable"|"auth_required",…}` in `-p` mode, stderr in `--serve` mode — and exits non-zero. Catalog harnesses whose routing is config-synthesis only (`opencode`, `pi-acp`, `hermes-acp`, `openclaw` — routed in the `bitrouter launch` interactive facet, not headless spawn yet) and non-catalog agents warn and run direct.
