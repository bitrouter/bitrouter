...
| `bitrouter status [--config PATH] [--socket PATH]` | `systemctl status`-style block: pid / listen / model count / the distinct providers behind them / socket. Reports `stopped` (exit 0) when no daemon is reachable. Same report type as the origin MCP server's `status` tool, so `--json` here and that tool's structured content are the same bytes. |
