...
| Command | Effect |
|---|---|
| `bitrouter route <model> [--prompt TEXT] [--config PATH]` | Resolve a model name through the routing table. Tries the running daemon first (live table), falls back to standalone config resolution — **policy table included**, so the answer is what would actually run. `--prompt` supplies the request text the policy table keys on (it routes by agent-loop step, so the selected model can differ with the prompt). JSON keys: `requested_model` (what you asked for) and `effective_model` (what would run — **read this one**), `effective_effort`, `resolved_via` (`live daemon` \| `config` \| `zero-config`), `policy_decision` (absent on `live daemon`: policy was applied upstream), `provider_chain[]` of `{provider, service_id, api_protocol}`, and `estimated_cost` (the first hop's per-token rates plus any steeper long-context brackets — rates, not a total). Same report type as the MCP `route_preview` tool. Read-only: nothing is sent upstream, and no credential is surfaced. |
| `bitrouter models [--config PATH] [--provider ID]` | List every routable model, each with **all** the providers that can serve it (the fallback chain, in order). Tries the running daemon first, falls back to a standalone config parse — same order as `bitrouter route`, and it matters: the live table carries subscription providers (`claude-code`, `google-ai`) whose credential is resolved at daemon start-up and which a config parse marks inactive and drops. `--json` reports `resolved_via: "live" \| "config"`. Filter with `--provider`. Same report type as the MCP `list_models` tool. |
