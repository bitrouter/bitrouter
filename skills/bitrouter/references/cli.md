...
| Command | Effect |
|---|---|
| `bitrouter tui --agent <id> [--worktree <name>]` | Launch the in-process multi-agent terminal UI: a fixed **left rail** (roster of every agent sorted by who needs you — needs-you `⚠` > attention `●` > running `⣷` > dead `✗` — plus a one-glyph-per-agent radar strip) beside a **splittable detail** view (1 agent by default, split to watch up to 4). Detail panes are titled `[slot] agent · harness · shortid`, stream messages/thoughts/tool-calls, take prompts at the `›` line, and answer permissions inline with `y`/`a`/`n`. `Ctrl-C` exits from any mode. Gated by the default-on `tui` feature. In-process (no daemon owns the session) — run `bitrouter serve` alongside so agent traffic routes through the proxy; the TUI probes the configured listen address at startup and shows a warning notice if nothing is listening, and a prompt that fails to reach an agent surfaces as a red `✗ prompt failed:` line in that pane. |
