...
| Command | Effect |
|---|---|
| `bitrouter tui --agent <id> [--worktree <name>]` | Launch the in-process multi-agent terminal UI: a fixed **left rail** (roster of every agent sorted by who needs you — needs-you `⚠` > attention `●` > working `⣷` > idle `○` > dead `✗` — plus a one-glyph-per-agent radar strip) beside a **splittable detail** view (1 agent by default, split to watch up to 4). Detail panes are titled `[slot] agent · harness · shortid · used/size · $cost`, stream messages/thoughts/tool-calls (streamed text commits per line — no word-per-line flicker; fenced code blocks are syntax-highlighted; tool diffs render `path +N/-M` headers with tinted `+`/`-` lines and `⋮` hunk gaps), take prompts at the `›` line, and answer permissions inline with `y`/`a`/`n`. `Ctrl-C` exits from any mode. Gated by the default-on `tui` feature. In-process (no daemon owns the session) — run `bitrouter serve` alongside so agent traffic routes through the proxy; the TUI probes the configured listen address at startup and shows a warning notice if nothing is listening, and a prompt that fails to reach an agent surfaces as a red `✗ prompt failed:` line in that pane. **Isolation**: the initial `--agent` runs in the base repo (`--worktree` opt-in), but every agent spawned from the picker gets its **own worktree + branch by default** (`.bitrouter/worktrees/<agent>-<record16>`, branch `bitrouter/<agent>-<record16>`, based on the manager's HEAD) plus a `PORT` from the `worktrees.ports` pool (default 3100–3199, shown as `:PORT` in the roster). Worktrees are **retained** on close — cleanup is gated on merged-or-discarded, never automatic. A configured `worktrees.bootstrap` shell hook runs in each newly created worktree (cwd = the worktree, `BITROUTER_BASE_REPO` and `PORT` in env); because it executes shell, the TUI shows the command in a CONFIRM overlay on first spawn each session — `y` run for the session / `n` skip / `Esc` cancel the spawn. |
| TUI modes | **NORMAL** (default): keys go to the focused detail pane; `PgUp`/`PgDn` scroll its scrollback (pinned view shows a `⇣N` newer-lines marker; paging back to the tail resumes following; works while a permission is pending); `Ctrl-A` → AGENT, `Ctrl-B` → BROADCAST. **AGENT** (`Ctrl-A`): `j`/`k`/`↑`/`↓` move the rail cursor · `Enter` open the cursor agent solo (returns to NORMAL) · `s`/`v` split the detail horizontally/vertically (max 4): adds the cursor agent, or — when the cursor agent is already shown, e.g. right after `Ctrl-A` — the most actionable agent not yet shown; a mode-bar notice explains full/no-candidate no-ops · `u` drop the focused slot · `Tab`/`←`/`→`/`1`-`4` switch detail-slot focus · `q` toggle queue focus (rail shows only agents that need you; empty = `✓ all clear`) · `y`/`a`/`d` resolve the cursor agent's pending permission from the rail (allow / allow-always / **d**eny — same pending the pane shows inline, so either surface clears both; rows with a pending expand to `└ <what it wants>`) · `A` cycle the cursor agent's autonomy tier — **manual** (default: everything surfaces) → **assisted** (low-risk auto-allows; high-risk surfaces) → **auto** (everything auto-allows) — shown on the row as `[a]`/`[A]`; every auto-allow is logged into the pane as an `· auto-allowed (…)` line, never silent; risk is classified from the tool call's kind + locations (reads/searches and writes inside the project tree = low; deletes, command execution, network, writes outside the tree, unknown = high) and expanded queue rows are tagged `high ·`/`low ·` with high-risk items ordered first · `n` new agent (picker) · `x` close the cursor agent · `Esc`. **COMMAND** (`:` on an empty prompt line, or `:` in AGENT mode): fuzzy palette over `spawn agent` / `close agent` / `split horizontal|vertical` / `unsplit` / `broadcast` / `queue` / `autonomy cycle` / `kill done` (closes every exited agent) / `keys help` / `quit`; type to filter (subsequence match) · `↑`/`↓` select · `Enter` run · `Esc`. `?` in AGENT mode (or the `keys help` command) shows a which-key popup listing the current mode's bindings; any key dismisses it. **PICKER**: `↑`/`↓` select · `Enter` spawn · `Esc`. **BROADCAST** (`Ctrl-B`): `Space` toggle the rail-cursor row · `1`-`9` toggle the Nth roster row · `a` select all · type · `Enter` send to selection · `Esc`. A background agent (not visible in the detail) needing attention rings the bell and shows `●` in the rail/radar. Cost HUD / detach are future work. |
