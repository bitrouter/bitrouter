...
Canonical model identifiers are slash-form ids such as **`openai/gpt-4o`**. A colon-form id such as **`openrouter:openai/gpt-4o`** is a deliberate provider pin: it routes directly through the named provider when that provider is active. Subscription providers are explicit-route-only, so `bro models` lists their usable selector in this pinned form instead of advertising a bare canonical selector that would not opt into the subscription. The exact selector strings come from the live routing table or your config — `bro models` is authoritative, and each displayed selector can be passed unchanged to `bro route`.
...
```bash
bro mcp check                 # MCP server liveness, latency, capabilities, tools
