...
impl App {
        ...
        match cmd {
            ...
            SlashCommand::SessionClear => {
                if let Some(sb) = self.state.active_scrollback_mut() {
                    *sb = crate::tui::model::ScrollbackState::new();
                }
            }
    ...
    fn slash_agents_update(&mut self, id: Option<String>, bitrouter_config: &BitrouterConfig) {
        let paths = self.acp_paths();
        let records = bitrouter::acp::state::load_state_sync(&paths.agent_state_file);
        if records.is_empty() {
    ...
    fn slash_import(&mut self, target: Option<(String, String)>) {
        ...
        if let Some((agent_id, external_id)) = target {
            ...
            let scanned = bitrouter::acp::session_import::scan_for_cwd(
                &home,
                &cwd,
        ...
        let mut items: Vec<crate::tui::model::PickerItem> = Vec::with_capacity(entries.len());
        let mut selectables: Vec<crate::tui::model::ImportCandidate> = Vec::new();
        for entry in entries {
            match entry {
                crate::tui::model::ImportEntry::Group { agent_id, count } => {
                    items.push(crate::tui::model::PickerItem {
                        label: format!("─── {agent_id} ({count})"),
                        subtitle: None,
                        selectable: false,
                    });
                }
                crate::tui::model::ImportEntry::Item(c) => {
                    let label = c
                        .title_hint
                        .clone()
        ...
        let mut aligned: Vec<crate::tui::model::ImportCandidate> = Vec::with_capacity(items.len());
        ...
        self.open_picker(
            ...
            items,
            crate::tui::model::PickerAction::Import {
                candidates: aligned,
            },
            true,
        );
        ...
        if self.state.session_store.active.is_empty() {
            ...
            for (i, s) in self.state.session_store.active.iter().enumerate() {
                ...
                let imported = match &s.source {
                    crate::tui::model::SessionSource::Imported { .. } => "↓ ",
                    crate::tui::model::SessionSource::Native => "  ",
                };
                lines.push(format!(
    ...
    fn slash_session_new(&mut self, agent: Option<String>) {
        ...
        let items: Vec<crate::tui::model::PickerItem> = self
            .state
            .agents
            .iter()
            .map(|a| {
                let subtitle = match &a.status {
                    crate::tui::model::AgentStatus::Idle => "installed",
                    crate::tui::model::AgentStatus::Available => "needs install",
                    crate::tui::model::AgentStatus::Installing { .. } => "installing",
                    crate::tui::model::AgentStatus::Connecting => "connecting",
                    crate::tui::model::AgentStatus::Connected => "connected",
                    crate::tui::model::AgentStatus::Busy => "busy",
                    crate::tui::model::AgentStatus::Error(_) => "error",
                };
                crate::tui::model::PickerItem {
                    label: a.name.clone(),
        ...
        self.open_picker(
            ...
            items,
            crate::tui::model::PickerAction::NewSession { agents },
            false,
        );
    }
    ...
    fn slash_session_switch(&mut self, id: Option<u64>) {
        if let Some(target_id) = id {
            let sid = crate::tui::model::SessionId(target_id);
        ...
        let ids: Vec<crate::tui::model::SessionId> = self
            .state
            .session_store
            .active
            .iter()
        ...
        self.open_picker(
            ...
            items,
            crate::tui::model::PickerAction::SwitchSession { ids },
            false,
        );
    }
    ...
    fn slash_obs(&mut self) {
        ...
        for a in &self.state.agents {
            let status = match &a.status {
                crate::tui::model::AgentStatus::Idle => "idle".to_string(),
                crate::tui::model::AgentStatus::Available => "available".to_string(),
                crate::tui::model::AgentStatus::Installing { percent } => {
                    format!("installing {percent}%")
                }
                crate::tui::model::AgentStatus::Connecting => "connecting".to_string(),
                crate::tui::model::AgentStatus::Connected => "connected".to_string(),
                crate::tui::model::AgentStatus::Busy => "busy".to_string(),
                crate::tui::model::AgentStatus::Error(m) => format!("error: {m}"),
            };
        ...
        for ev in self.state.obs_log.events.iter().rev().take(50) {
            let kind = match &ev.kind {
                crate::tui::model::ObsEventKind::Connected => "connected".to_string(),
                crate::tui::model::ObsEventKind::Disconnected => "disconnected".to_string(),
                crate::tui::model::ObsEventKind::PromptSent => "prompt sent".to_string(),
                crate::tui::model::ObsEventKind::PromptDone => "prompt done".to_string(),
                crate::tui::model::ObsEventKind::ToolCall { title } => format!("tool: {title}"),
                crate::tui::model::ObsEventKind::Error { message } => format!("error: {message}"),
            };
...
fn placeholder_candidate() -> crate::tui::model::ImportCandidate {
    crate::tui::model::ImportCandidate {
        agent_id: String::new(),
