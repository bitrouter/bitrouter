...
use bitrouter::runtime::{AppRuntime, PathOverrides, RuntimePaths, resolve_home};
...
enum Command {
    ...
    /// Launch the interactive terminal UI (multi-agent ACP session manager)
    #[cfg(feature = "tui")]
    Tui,
}
...
async fn run_cli(cli: Cli) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Handle reset: confirm, wipe config, re-run onboarding.
    if matches!(cli.command, Some(Command::Reset)) {
        return run_reset(&paths).await;
    }

    // Bare `bitrouter` (no subcommand) — headless-first behavior:
    //   * unconfigured → run the interactive `init` wizard, then exit so the
    //     operator can decide what to start next.
    //   * configured   → fall through to the server path (equivalent to
    //     `bitrouter serve`). The TUI is opt-in via `bitrouter tui` or the
    //     `tui` feature; it never auto-launches.
    let bare_serve = if cli.command.is_none() {
        ...
        if !config_exists {
            ...
            if outcome == init::InitOutcome::Configured {
                println!();
                println!("  Run `bitrouter` to start the proxy in the foreground,");
                println!("  or `bitrouter start` to start it as a background daemon.");
                println!();
            }
            return Ok(());
        }
    ...
    if (bare_serve || is_server_command(&cli.command))
    ...
    if bare_serve {
        return run_serve(cli.database_url.as_deref(), runtime, &paths).await;
    }

    match cli.command {
        Some(Command::Serve) => run_serve(cli.database_url.as_deref(), runtime, &paths).await?,
...
/// Start the BitRouter proxy in the foreground (the `serve` code path, also
/// taken by a bare `bitrouter` invocation when a configuration already exists).
async fn run_serve(
    ...
    let db_url = bitrouter::runtime::resolve_database_url(
        database_url,
        &runtime.config,
        &paths.home_dir,
        env_file,
    ...
    let db = sea_orm::Database::connect(db_opts).await.map_err(|e| {
        bitrouter::runtime::error::RuntimeError::Other(format!(
            "database connection failed: {e}. Check `database.url` in {} or BITROUTER_DATABASE_URL.",
            paths.config_file.display()
        ))
    })?;
    bitrouter::runtime::migrate(&db).await.map_err(|e| {
        bitrouter::runtime::error::RuntimeError::Other(format!("database migration failed: {e}"))
    })?;
    let db = Arc::new(db);

    print_first_run_guidance(&runtime);
    let base_client = bitrouter::runtime::http_client::build_upstream_client();
    let client_builder = reqwest_middleware::ClientBuilder::new(base_client);
    let client_builder =
        match bitrouter::runtime::payment::build_payment_middleware(&runtime.config) {
    ...
    let model_router =
        bitrouter::runtime::Router::new(client_builder.build(), runtime.config.providers.clone())
...
/// need the OWS wallet passphrase resolved up front. `Restart` shuts the
/// running daemon and spawns a fresh one, so it needs the same prompt.
fn is_server_command(cmd: &Option<Command>) -> bool {
    matches!(
        cmd,
        Some(Command::Serve | Command::Start | Command::Restart),
    )
}
...
/// Reset configuration and re-run the setup wizard. The wizard exits after
/// configuration is saved; the operator can then re-run `bitrouter` to start
/// the proxy (or `bitrouter tui` for the interactive UI).
async fn run_reset(paths: &RuntimePaths) -> Result<(), Box<dyn std::error::Error>> {
...
/// Launch the interactive TUI (the multi-agent ACP session manager). Invoked
/// only by `bitrouter tui`; never auto-launched.
#[cfg(feature = "tui")]
async fn run_tui(paths: &RuntimePaths) -> Result<(), Box<dyn std::error::Error>> {
    let runtime: DefaultRuntime = load_or_warn_scaffold(paths);
    let status = runtime.status();
    ...
    // Merge the ACP registry into the agent set so first-render discovery
    // sees the full catalog. A failed fetch falls through silently — the
    ...
    crate::tui::run(tui_config, &bitrouter_config, launch_cwd).await?;
    Ok(())
}
