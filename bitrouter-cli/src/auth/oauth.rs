...
use bitrouter::auth::token_store::{OAuthToken, TokenStore};
...
/// conventions (see [`bitrouter::runtime::copilot::enterprise_urls`]).
pub fn params_from_oauth_config(
    ...
    let (default_device_url, default_token_url) = match domain {
        Some(d) if !d.is_empty() && d != "github.com" => {
            let (dev, tok, _api) = bitrouter::runtime::copilot::enterprise_urls(d);
            (dev, tok)
        }
        _ => (
