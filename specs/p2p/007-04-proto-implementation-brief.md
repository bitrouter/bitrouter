...
- Leg B 已实现独立 Control Connection：`bitrouter/payctl/1`、JCS control frames、cumulative voucher、epoch close、PGW restart recovery。
...
| Leg B Data/Control split | 通过 | PGW 数据面只用 `BR-Order-Ref`；控制面使用 `bitrouter/payctl/1` JCS frames |
