...
flowchart LR
    ...
    Consumer -- "Leg A (Direct)<br/>BitRouter 协议<br/>ALPN bitrouter/p2p/0 = HTTP/3" --> Provider
    Consumer -- "Leg C (PGW-fronted)<br/>**不在 BitRouter 协议范围**<br/>HTTPS 公网出口" --> PGW
    PGW      -- "Leg B (Provider link)<br/>Data ALPN h3 + Control ALPN bitrouter/payctl/1" --> Provider
```

| Leg | 端到端 | 协议归属 | 主文档 |
|---|---|---|---|
| **Leg A** | Consumer ↔ Provider（Pure P2P 直连） | ==BitRouter 协议范围内==；ALPN `bitrouter/p2p/0` 上承载 HTTP/3（§6.1）；MPP wire 见 [`005`](./005-l3-payment.md) | 005 |
| **Leg B** | PGW ↔ Provider（PGW 路径上的内部一跳） | ==BitRouter 协议范围内==；==dual-connection 拓扑==——**Data Connection** 走标准 ALPN `h3` (HTTP/3) 承载 LLM 流；**Control Connection** 走 ALPN `bitrouter/payctl/1` 承载长度前缀 JCS-JSON 支付控制帧（**非** WebSocket）；详见 [`004-03`](./004-03-pgw-provider-link.md) | 004-03 |
...
- Leg A 与 Leg B **不**复用同一 ALPN——历史上 `bitrouter/p2p/0` 曾被理解为通用 P2P，v0.7 起明确**仅**用于 Leg A。Leg B 的 Data Connection 切到 IETF 标准 `h3` ALPN，避免 BitRouter "假装" HTTP/3 的兼容性问题。
- Leg B 的 Control Connection 与 Data Connection **必须是两条独立 QUIC connection**：Data Connection 使用 ALPN `h3`，Control Connection 使用 ALPN `bitrouter/payctl/1`。二者绑定到同一对 (PGW endpoint, Provider endpoint) 与同一长期 BR-internal payment channel，但不能在同一 QUIC connection 内复用 stream。
...
{
  // —— 身份 ——
  "provider_id": "ed25519:<52-z-base32>",       // root pubkey；主键；永不变更
  ...
  "valid_until": "2026-07-01T00:00:00Z",        // 到期 = Registry/合约不再下发；强制 root 重签
  "snapshot_hash_alg": "sha256-canonical-json", // v0 与 v1 使用同一哈希口径

  // —— 实例集（可多实例并存）——
  "endpoints": [
    {
      "endpoint_id": "ed25519:<...>",            // iroh EndpointId（热密钥）
      "region": "geo:ap-east-1",
      "node_addr": {                             // 可选；缺省走 pkarr
    ...
    // 多个 endpoint = 横向扩容（详见 006）
  ],

  // —— 经济参数（provider 级，所有 endpoint 共享）——
  "models": [
    {
  ...
  // —— 完整性 ——
  "sig": "ed25519:<root-key-sig over canonical_json(snapshot \\ sig)>"
}
```
...
- **签名覆盖整个 snapshot**——`endpoints[]` 的真实性、pricing 真实性、accepted_pgws 真实性都派生自 root sig，==**不需要每个 endpoint 单独委托签名**==。
...
1. JSON Schema lint。
2. ==`sig` 验证==（root key 签 canonical_json）。
...
> ==**HTTP 服务本身不可信**==——所有下发的 snapshot 都带 root key `sig`，调用方应离线用 `provider_id` / `pgw_id` 验证。该服务在 v0 仅作"分发与索引便利层"；v1 上链后被链上 RPC + 链下 cache 取代（§8.4）。
...
{
  "providers": [
    { /* 完整 snapshot，schema 见 §2.2，含 sig */ }
  ],
...
- ==**始终下发完整 snapshot**==（含 `sig`）——上层可独立验。Registry 服务无需被信任。
- 默认按总价升序；过滤参数仅起"减小返回集"作用，不强制选谁。
- `region` 过滤是对 `endpoints[].region` 的过滤；服务侧返回的 snapshot 仍带原始全部 `endpoints[]`（避免破坏 sig 校验），由 Consumer 在客户端按 region 选实例。
- 限速：匿名 IP 60 req/min。
...
返回完整 snapshot（含 `sig`）。Consumer 拨号失败后用来确认 `endpoints[]` / `seq` 是否变更。

### 3.3 仓库变更如何生效

```mermaid
flowchart LR
    Build["运维生成新 snapshot<br/>(seq+1, root sig)"] --> PR["GitHub PR"]
    PR --> CI["CI: 验 sig / seq 单调 / schema"]
...
- 紧急下架：生成 `status: "suspended"` 的新 snapshot（**root key 必须签**）+ PR。==root key 离线时无法下架——这是协议级折衷==（v1 直接照搬：合约 require root sig）。
- v1 切换：把"PR + merge"换成"提交链上交易"，CI 检查改成合约校验；snapshot 内容、sig 算法、`seq` 语义不变。
...
sequenceDiagram
    ...
    LR->>Prov: iroh.connect_addr(node_addr_B)<br/>(QUIC 建立, ALPN=bitrouter/p2p/0)
...
### 6.1 Framing：HTTP/3 in QUIC（ALPN `bitrouter/p2p/0` 重定义）

==**v0.7（R2 决议）起的协议级决策**==：ALPN `bitrouter/p2p/0` **重定义**为承载**标准 HTTP/3** 的 BitRouter 命名空间——不再使用 "HTTP/1.1 在裸 QUIC bidi stream 上跑" 的早期方案。每个 P2P 请求 = 一条 HTTP/3 request stream（QPACK 头压缩、HEADERS / DATA frame、SETTINGS、CONTROL stream、push 禁用），对端按完整 HTTP/3 协议栈处理（[RFC 9114](https://www.rfc-editor.org/rfc/rfc9114)）。

==**关键设计要点**==：

- **单一 ALPN**：`bitrouter/p2p/0` 是 Leg A 的**唯一** ALPN；Leg B 用 `h3` + `bitrouter/payctl/1`（详见 §1.4 / [`004-03`](./004-03-pgw-provider-link.md)）。
...
| `<auth-and-payment-settlement>` | P→C HTTP trailer | **占位**——具体格式见 005（trailer 名 `Payment-Receipt` / `Payment-Receipt-Sig`，fallback `GET /v1/payments/receipts/{id}`） |
...
| `400` / `404` / `409` / `429` / `5xx` | 非支付错误 | `application/problem+json`（[RFC 9457](https://www.rfc-editor.org/rfc/rfc9457)） | 按 `type` URI 处理 |
| `409` + `X-Bitrouter-Quote-Version` | 报价版本陈旧 | problem+json + 扩展字段 | 回 Registry 拉新报价、重选 |
| `429 Too Many Requests` | Provider 限流 | problem+json | 退到候选池下一个 |
| `5xx` | Provider 内部错 | problem+json | 候选池下一个；记一次 failure |
...
L1 的 ALPN `bitrouter/p2p/0` 表达 "这是 Leg A 的 BitRouter HTTP/3 通道"（§6.1），**不**表达 L3 版本。
...
flowchart LR
    ...
    B --> C["L3: snapshot 完整性<br/>provider_id root sig 校验"]
...
2. **自包含 + 自验证**：snapshot 内的所有字段（含 `endpoints[]`）都被同一个 root sig 覆盖；任何节点离线即可验，无需信任 Registry 服务。
3. **状态版本由 `seq` 单调表达**：v0 CI / v1 合约都强制 `require(new_seq > old_seq)`——用于回放保护。
4. **哈希算法固定**：`sha256(canonical_json(snapshot \ sig))`——v0 用作 git diff hint / v1 用作 on-chain commitment。
...
| 写入授权 | git push 权限 + PR review | tx 由 root key 签名；合约校验 `ecrecover(sig) == provider_id` |
...
| **B**：commit-on-chain + data-off-chain（==**推荐**==） | 链上仅存 `{provider_id, snapshot_hash, snapshot_uri, seq, sig}`；snapshot 体本身放 IPFS / arweave / 任意 S3 | gas 成本恒定；保留可验证性；存储成本可承受 | 需要解决 data availability（snapshot URI 失联问题，可由多镜像缓解） |
...
2. **谁有 commit 权限**？建议 protected branch + 至少 1 个团队成员 review；同时 CI 强制 root sig 校验（任何条目变更都必须由对应 root key 签）——保证即使 commit 权限被滥用也无法伪造他人 snapshot。
...
2. **`bitrouter-registry` 仓库**建仓 + CI（JSON Schema lint + root sig 校验 + seq 单调校验）。
3. **Registry HTTPS 服务**单实例上线（拉 git → 内存索引 → 只读 API；下发完整 snapshot 含 sig）。
...
6. **BitRouter 主仓扩 Local Router**：让 candidate trait 能装 P2P 候选；接入 Registry Agent；客户端验 root sig。
...
### 2. 非支付类错误 → RFC 9457 `application/problem+json`

- 触发条件：参数缺失 / 模型不存在 / 报价版本陈旧 (`X-Bitrouter-Quote-Version`) / 限流 / 内部错 / 上游模型超时等。
- HTTP 响应：标准 status code（`400` / `404` / `409` / `429` / `5xx`）+ body `Content-Type: application/problem+json`，schema 严格遵守 [RFC 9457](https://www.rfc-editor.org/rfc/rfc9457)：

  ```jsonc
  {
    "type":     "https://docs.bitrouter.ai/errors/quote-version-mismatch",
    ...
    // —— BitRouter 扩展字段（RFC 9457 §3.2 允许扩展） ——
    "br_request_id": "01HG..."
  }
  ```

- 节点实现层应当为每个常用错误维护一个稳定的 `type` URI（用于客户端 switch / 监控分类）；HTTP `title` 与 `detail` 之间按 RFC 9457 区分（title = 不变的简称、detail = 本次实例描述）。

### 3. SSE 流内错误 → `data: {"error": {...}}` + `[DONE]`

- 触发条件：HTTP/3 已经返回 `200 OK` + 开始 SSE 流，但流中途出错（上游模型断开、Provider 内部异常等）。
- 协议：发送一条 `data: {"error": {"message": "...", "type": "...", "code": "..."}}\n\n` 帧，紧接 `data: [DONE]\n\n`，关闭 stream。错误 schema 沿用 OpenAI v1 chat completions 的 `error` 子对象（详见 [`005 §2`](./005-l3-payment.md) / R3）。
- ==**禁止**在 SSE 流内插入 `event:` 字段==——所有 frame 都是匿名 `data:` 帧，对 OpenAI / Anthropic 兼容客户端透明。

### 4. 跨 leg 适用性

| Leg | 1（402+MPP） | 2（problem+json） | 3（SSE 内） |
|---|---|---|---|
...
| Leg B Control Connection（`bitrouter/payctl/1`） | ❌（控制平面用专用 `payment-error` 帧，详见 [`004-03`](./004-03-pgw-provider-link.md)） | ❌ | ❌ |
