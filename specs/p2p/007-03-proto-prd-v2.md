...
- **传输层标准化**：Leg A 使用 ALPN `bitrouter/p2p/0` + HTTP/3 语义；Leg B 数据连接使用标准 ALPN `h3`；Leg B 控制连接使用独立 QUIC ALPN `bitrouter/payctl/1`。不再使用第一版的自定义请求 / 响应分帧。
...
| 类别        | 第二版必须实现                                                                                                                                |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Registry  | 继续使用中心化 git 仓库 + 只读服务；snapshot 字段按最新 [`003`](./003-l3-design.md) / [`001-02`](./001-02-terms.md)；身份字符串统一 `ed25519:<z-base32>`          |
...
flowchart LR
    ...
    Consumer -- "Leg A<br/>ALPN bitrouter/p2p/0<br/>HTTP/3 + MPP" --> Provider
    Consumer -- "Leg C<br/>HTTPS OpenAI-compatible" --> PGW
    PGW -- "Leg B Data<br/>ALPN h3<br/>BR-Order-Ref only" --> Provider
    PGW -- "Leg B Control<br/>ALPN bitrouter/payctl/1<br/>cumulative voucher" --> Provider
...
2. Consumer 用 iroh 连接 Provider：ALPN `bitrouter/p2p/0`。
3. 连接内跑 HTTP/3 请求：
...
8. Provider 在 HTTP trailer 发送：
   ...
   Payment-Receipt-Sig: <base64url(provider_ed25519_sig)>
   ```
...
| 控制连接 | 独立 QUIC 连接；ALPN `bitrouter/payctl/1`; length-prefixed JCS-JSON frames |
...
- 所有 BitRouter node identity 使用 [`001-02 §8.5`](./001-02-terms.md)：`<algo>:<z-base32-lower-no-pad>`。
...
- `endpoints[].alpn`：
  - Leg A Provider endpoint: `bitrouter/p2p/0`;
  - PGW / Provider Leg B Data: `h3` 可作为能力字段或在 PGW config 中声明；
  - Control ALPN `bitrouter/payctl/1` 必须在 PGW/Provider config 中声明。
...
| 模块 | 要求 |
|---|---|
| Identity | root ed25519 + endpoint ed25519；z-base32 encoding；snapshot sig |
| iroh | 自定义中继配置；Leg A 使用 ALPN `bitrouter/p2p/0`；Leg B 使用 ALPN `h3` 和 `bitrouter/payctl/1` |
...
| Registry client | 拉 Provider snapshots，验证 root sig |
...
- z-base32 identity regex and length checks.
- rational pricing schema.
- base-unit integer string schema.
- `endpoints[].alpn == "bitrouter/p2p/0"` for Leg A Provider endpoint.
...
| 测试 | 配置 | 组件 | 目的 |
|---|---|---|---|
| unit-protocol | unit | 无链 | JCS、z-base32、rational、base-unit schema |
