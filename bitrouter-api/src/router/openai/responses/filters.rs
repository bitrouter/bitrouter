...
///
/// The filter accepts POST requests with a JSON body in OpenAI Responses format,
    ...
    R: LanguageModelRouter + Send + Sync + 'static,
{
    ...
    if is_stream {
        let result = handle_stream(&model, options).await;
    ...
    R: LanguageModelRouter + Send + Sync + 'static,
{
    ...
    handle_responses(request, table, hooked, metrics).await
}
