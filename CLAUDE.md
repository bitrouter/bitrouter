...
5. **bitrouter** (binary) — The CLI product. Depends on `bitrouter-core`, `bitrouter-config`, `bitrouter-api` and all provider crates. Assembles everything: resolves paths, loads config, and provides the user-facing commands (`serve`, `start`, `stop`, `status`, `restart`) and optional TUI.
