...
The surface is **control and introspection only** — it runs no inference.
Completions go to the daemon's HTTP API (`/v1/messages`,
`/v1/chat/completions`), which is the transport built for them: streaming, the
full parameter surface, and the metering path included.

Every tool comes through one of the crate's ports — `StatusQuery`,
`ModelsQuery` and `RouteQuery` (in `actions/`), `SkillsQuery` (in
`capabilities/`) — so the crate itself stays substrate-free. The host-bound ones
are wired on **stdio only**: they read the serving machine's own routing table
and skill library, which has no meaning on a multi-tenant HTTP transport.
`status` and `list_models` are the exception to *that*: a backend that genuinely
knows the answer for its own deployment hands its port over
...
(`bro status` / `models` / `route`), listed in `actions::ACTIONS` and
held there by a guard test in `apps/bitrouter`. See
[`docs/ACTIONS_SPEC.md`](../../docs/ACTIONS_SPEC.md).

| Tool | Wired on | Description |
|------|----------|-------------|
| `list_models` | all | Every routable model with **all** the providers that can serve it — the fallback chain, not just the first hop. Optional `provider` filter. Returns the shared `actions::models::ModelsReport`, so the tool advertises an `output_schema` and `bro models --json` is the same bytes. `resolved_via` distinguishes a running router's catalog (`live`) from a static-config projection (`config`); on stdio + local the app-injected port falls back to the latter, so the tool answers with no daemon running |
...
whose machine it is, so a `bro mcp install`-ed client sees them.
...
│   ├── backend/        # Backend trait (port handover) + LocalBackend / CloudBackend
