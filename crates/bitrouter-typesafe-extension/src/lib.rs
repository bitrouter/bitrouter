//! Native TypeSafe System One JSON facet for explicitly linked custom hosts.
//!
...
pub const EXTENSION_ID: &str = "typesafe";
/// The facet scoped to [`EXTENSION_ID`].
pub const ADAPTER_ID: &str = "system_one";
/// Contract revision used by the TypeSafe provider route.
...
/// TypeSafe System One's non-streaming JSON dialect.
    ...
    fn render_request(
        ...
        if target.provider_model_id.is_empty() {
            return Err(BitrouterError::bad_request(
                "TypeSafe provider model id must be non-empty",
            ));
        }
        Ok(json!({
...
fn invalid_answer() -> BitrouterError {
    BitrouterError::UpstreamInvalidResponse {
        message: "TypeSafe returned a malformed evaluation answer".to_owned(),
    }
}
