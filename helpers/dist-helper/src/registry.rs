...
/// Regenerate (or check) the model/provider tables in the `supported-*` docs.
///
/// Source of truth is the freshly built registry catalog (same data as
/// `dist/registry/{models,providers}.json`); only the table block under a fixed
/// anchor heading is rewritten, so the surrounding hand-written prose is
/// preserved. The English and Chinese pages share identical data rows — only the
/// header row differs — keeping the translations in lockstep.
pub fn docs(root: &Path, check: bool) -> Result<()> {
    let artifacts = build_artifacts(root)?;
    let models: Value =
        serde_json::from_str(&artifacts.models).context("parsing built models catalog")?;
    let providers: Value =
        serde_json::from_str(&artifacts.providers).context("parsing built providers catalog")?;
    let empty = Vec::new();
    let model_rows = model_rows(models["data"].as_array().unwrap_or(&empty));
    let provider_rows = provider_rows(providers["data"].as_array().unwrap_or(&empty));

    let mut stale = Vec::new();
    for target in DOC_TARGETS {
        let rows = if target.providers {
            &provider_rows
        } else {
            &model_rows
        };
        ...
        let updated = replace_table_block(&original, target.anchor, target.header, rows)
            .with_context(|| format!("rewriting table in {}", target.path))?;
        if original == updated {
            continue;
        }
        if check {
    ...
    if check {
        if !stale.is_empty() {
            bail!(
                "supported-* docs tables are stale - run `cargo run -p dist-helper -- registry docs` and commit docs/get-started:\n  - {}",
                stale.join("\n  - ")
            );
        }
        println!(
            "supported-* docs tables are up to date: {} models, {} providers",
            model_rows.len(),
            provider_rows.len()
        );
    } else {
        println!(
            "wrote supported-* docs tables: {} models, {} providers across 4 pages",
            model_rows.len(),
            provider_rows.len()
        );
    }
...
/// Replace the Markdown table under `anchor` with a freshly rendered one,
/// preserving everything before the anchor's following blank line and from the
/// next `## ` heading onward. Mirrors the previous `gen-supported-tables.py`.
fn replace_table_block(
    original: &str,
    anchor: &str,
    header: &str,
    rows: &[String],
    ...
    let start = lines
        ...
        .with_context(|| format!("anchor {anchor:?} not found"))?;
    ...
    out.push(String::new());
    out.push(render_table(header, rows));
    out.push(String::new());
...
fn render_table(header: &str, rows: &[String]) -> String {
    ...
    let mut lines = vec![header.to_string(), separator];
    lines.extend(rows.iter().cloned());
    lines.join("\n")
}

fn doc_cell(value: &str) -> String {
    value.replace('|', "\\|")
}

/// Render a USD-per-1M price the way `%g` did: shortest form, no trailing zeros,
/// an em dash for the absent case.
...
/// Common context windows mix decimal and binary conventions; map the known
/// sizes to their conventional label, and fall back to decimal K/M otherwise.
fn ctx_label(tokens: Option<u64>) -> String {
    let Some(n) = tokens.filter(|n| *n != 0) else {
        return "—".to_string();
    };
    let known = match n {
        128_000 | 131_072 => Some("128K"),
        196_608 => Some("192K"),
        200_000 => Some("200K"),
        202_752 => Some("198K"),
        256_000 | 262_144 => Some("256K"),
        524_288 => Some("512K"),
        1_000_000 | 1_048_576 => Some("1M"),
        2_000_000 | 2_097_152 => Some("2M"),
        4_194_304 => Some("4M"),
        _ => None,
    };
...
fn best_price(model: &Value) -> (Option<f64>, Option<f64>) {
    let mut best: Option<(f64, Option<f64>)> = None;
    for provider in model["providers"].as_array().into_iter().flatten() {
        let pricing = &provider["pricing"];
        let Some(input) = pricing["input_tokens"]["no_cache"].as_f64() else {
            continue;
        };
        let output = pricing["output_tokens"]["text"].as_f64();
        if best.is_none_or(|(cheapest, _)| input < cheapest) {
            best = Some((input, output));
        }
    }
    match best {
...
fn model_rows(models: &[Value]) -> Vec<String> {
    models
        .iter()
        .map(|model| {
            ...
            let modalities = if modalities.is_empty() {
                ...
                doc_cell(&modalities.join(", "))
            };
            let open = if model["open_weights"].as_bool().unwrap_or(false) {
                "✅"
            } else {
                "—"
            };
            let (input, output) = best_price(model);
            format!(
                "| `{id}` | {} | {} | {modalities} | {open} | {} | {} |",
                doc_cell(name),
                ctx_label(model["max_input_tokens"].as_u64()),
                usd(input),
                usd(output),
            )
        })
...
fn provider_rows(providers: &[Value]) -> Vec<String> {
    providers
        .iter()
        .map(|provider| {
            let id = provider["id"].as_str().unwrap_or_default();
            let metadata = &provider["metadata"];
            let name = metadata["name"].as_str().unwrap_or(id);
            let headquarters = metadata["headquarters"].as_str().unwrap_or("—");
            let mut protocols = BTreeSet::new();
            for model in provider["models"].as_array().into_iter().flatten() {
                match &model["api_protocol"] {
                    Value::String(protocol) => {
                        protocols.insert(protocol.clone());
                    }
                    Value::Array(list) => {
                        for protocol in list.iter().filter_map(Value::as_str) {
            ...
            let protocols = if protocols.is_empty() {
                ...
                doc_cell(&protocols.into_iter().collect::<Vec<_>>().join(", "))
            };
            let billing = match provider["billing"].as_str() {
                Some("usage_token") => "Per-token".to_string(),
                Some("subscription") => "Subscription".to_string(),
                Some(other) => doc_cell(other),
                None => "—".to_string(),
            };
            let count = provider["models"].as_array().map_or(0, Vec::len);
            format!(
                "| `{id}` | {} | {} | {protocols} | {billing} | {count} |",
                doc_cell(name),
                doc_cell(headquarters),
            )
        })
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn ctx_label_uses_known_sizes_then_decimal_fallback() {
        assert_eq!(ctx_label(None), "—");
        assert_eq!(ctx_label(Some(0)), "—");
        assert_eq!(ctx_label(Some(200_000)), "200K");
        assert_eq!(ctx_label(Some(1_048_576)), "1M");
        assert_eq!(ctx_label(Some(119_808)), "120K");
        assert_eq!(ctx_label(Some(1_500_000)), "1.5M");
    }

    #[test]
    fn usd_renders_shortest_form_like_percent_g() {
        assert_eq!(usd(None), "—");
        assert_eq!(usd(Some(10.0)), "$10");
        assert_eq!(usd(Some(22.5)), "$22.5");
        assert_eq!(usd(Some(0.315)), "$0.315");
    }

    #[test]
    fn doc_rows_render_from_built_catalog_shape() {
        let models = json!([
            {
                "id": "acme/one",
                "name": "Acme: One",
                ...
                "open_weights": true,
                "providers": [
                    { "pricing": { "input_tokens": { "no_cache": 3.0 }, "output_tokens": { "text": 15.0 } } },
                    { "pricing": { "input_tokens": { "no_cache": 1.0 }, "output_tokens": { "text": 5.0 } } }
                ]
            }
        ]);
        let rows = model_rows(models.as_array().unwrap());
        assert_eq!(
            rows,
            vec!["| `acme/one` | Acme: One | 200K | text, image | ✅ | $1 | $5 |".to_string()]
        );

        let providers = json!([
            {
                "id": "acme",
                "metadata": { "name": "Acme", "headquarters": "US" },
                "billing": "usage_token",
                "models": [
                    { "api_protocol": "openai" },
        ...
        let rows = provider_rows(providers.as_array().unwrap());
        assert_eq!(
            rows,
            vec!["| `acme` | Acme | US | anthropic, openai | Per-token | 2 |".to_string()]
        );
    }

    #[test]
    fn replace_table_block_swaps_only_the_anchored_table() {
        let original =
            "# Title\n\n## Model catalog\n\n| old |\n| --- |\n| stale |\n\n## Next\n\nkeep me\n";
        let updated = replace_table_block(
            original,
            "## Model catalog",
            "| Model |",
            &["| `acme/one` |".to_string()],
